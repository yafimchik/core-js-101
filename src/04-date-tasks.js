...
function timeSpanToString(startDate, endDate) {
  ...
  let sss = (d.getUTCMilliseconds() < 100) ? `0${d.getUTCMilliseconds()}` : `${d.getUTCMilliseconds()}`;
  sss = (sss.length < 3) ? `0${sss}` : `${sss}`;
  return `${hh}:${mm}:${ss}.${sss}`;
}
...
function angleBetweenClockHands(date) {
  let h = (date.getUTCHours() % 12) * 60;
  let m = date.getUTCMinutes();
  h += m;
  m *= 12;
  const angle = Math.min(Math.abs(h - m), Math.abs(720 - h + m));
