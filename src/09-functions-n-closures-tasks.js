...
function getPolynom(...args) {
  ...
  const args2 = args;
  function f(x) {
    return args2.map((a, i) => a * x ** (args2.length - 1 - i)).reduce((b, c) => b + c);
  }
  return f;
}
...
function logger(func, logFunc) {
  function f(...args) {
    const argT = JSON.stringify(args).slice(1, JSON.stringify(args).length - 1);
    f.log(`${f.func.name}(${argT}) starts`);
    const res = f.func(...args);
    f.log(`${f.func.name}(${argT}) ends`);
    return res;
  }
  f.func = func;
  f.log = logFunc;
